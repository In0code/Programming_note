		...
		if("1".equals(request.getParameter("dataFlag"))){
			if("leesw".equals(id)){
				...
				for(int i=0; i < list.size(); i++){
					list.get(i).setName(dd.decryption(list.get(i).getName()));  // VO
...
<div>
<form method="post" id="frm" enctype="application/x-www-form-urlencoded">
...
<a href="member_list.jsp?dataFlag=1" class="btn btn-success" style="margin-top:3px">전체보기</a>
...
<input type="text" id="keyword" name="keyword" class="inputBox" value="${ param.keyword ne 'null'? param.keyword : ''  }"/>
<!-- <input type="text" style="display: none;"/> -->
<input type="hidden" name="dataFlag" value="1"/>
<input type="button" name="btnSearch" id="btnSearch" value="클릭하여 검색" class="btn btn-dark"/>
</form>
...
//구해진 끝 페이지 번호가 총 페이지보다 작다면 총 페이지수가 끝 페이지 번호가 된다.
...
while( movePage <= endPage ){
	if(movePage == currentPage){ //현재페이지와 이동할 페이지가 같다면 링크없이 인덱스 리스트 제공
		%>
		[ <span style="font-size:20px"><%= movePage %></span> ]
		<%
	}else{
		%>
